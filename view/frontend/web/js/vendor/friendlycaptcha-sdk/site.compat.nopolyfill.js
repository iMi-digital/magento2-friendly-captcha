...
(function () {
  ...
  function findFRCElements() {
    var captchaElements = document.querySelectorAll(".frc-captcha");
    var riskIntelligenceElements = document.querySelectorAll(".frc-risk-intelligence");
  ...
  var CommunicationBus = /*#__PURE__*/function () {
    ...
    _createClass(CommunicationBus, [{
      ...
      key: "addOrigins",
      value: function addOrigins(origins) {
        var _this2 = this;
        origins.forEach(function (origin) {
          return _this2.origins.add(origin);
        });
      }
      /**
  ...
  var Store = /*#__PURE__*/function () {
    ...
    _createClass(Store, [{
      ...
      value: function get(key) {
        var storeKey = this.storePrefix + SEPARATOR + key;
        try {
  ...
  function getLocalizedWidgetTitle(lang) {
    lang = getLanguageCode(lang);
    var messages = PLACEHOLDER_LOCALIZATIONS[lang] || PLACEHOLDER_LOCALIZATIONS["en"];
  ...
  function createBanner(opts) {
    ...
    a.onmouseenter = function () {
      return s.textDecorationColor = primaryColor;
    };
    a.onmouseleave = function () {
      return s.textDecorationColor = secondaryColor;
    };
    el.appendChild(a);
    opts.element.appendChild(el);
  }
  ...
  function createFallback(el, apiOrigin, language) {
    var link = function link(href, text) {
    ...
    var failedText = getLocalizedPlaceholderText(language, "failed");
    var els = [link("".concat(apiOrigin, "/connectionTest"), failedText)];
    el.textContent = "";
        ...
        if (this.startMode === "focus" && !this.focusEventPending && !this.isDestroyed) {
          ...
          if (formElement) {
            ...
            executeOnceOnFocusInEvent(formElement, function (ev) {
              _this4.trigger("focus", {
                ev: ev
              });
              _this4.focusEventPending = false;
            });
          }
    ...
    _createClass(Signals, [{
      ...
      value: function setupMovementMetrics() {
        ...
        var updateFunc = function updateFunc() {
          ...
          lastSample && (lastSample[0] && _this5.tm.timeStamp === lastSample[1] ||
          // Last sample was touch and timestamp is unchanged.
          !lastSample[0] && _this5.mm.timeStamp === lastSample[1])) {
          ...
          var evType = 0;
          if (lastSample) evType = lastSample[0];else if (_this5.mm && _this5.tm) evType = _this5.mm.timeStamp > _this5.tm.timeStamp ? 0 : 1;else if (!_this5.mm) evType = 1;
          if (evType) {
            var t = _this5.tm.touches[0];
            t && sample.push([1, _this5.tm.timeStamp, t.screenX, t.screenY]);
          } else {
            sample.push([0, _this5.mm.timeStamp, _this5.mm.screenX, _this5.mm.screenY]);
          }
        };
        var lastRadius = -1;
        runOnDocumentLoaded(function () {
  ...
  // src/sdk/options.ts
  var toFRCAPIUrl = function toFRCAPIUrl(hosts) {
    return hosts.map(function (h) {
      return "https://".concat(h, ".frcapi.com");
    }).join(",");
  };
  var SHORTHANDS = {
  ...
  var splitCSV = function splitCSV(value) {
  ...
  var expandEndpointShorthand = function expandEndpointShorthand(value) {
    return splitCSV(SHORTHANDS[value] || value);
  };
  function resolveAPIOrigins(optionValue) {
    var endpointList = optionValue || SHORTHANDS.global;
    var resolved = splitCSV(endpointList).reduce(function (acc, endpoint) {
      return acc.concat(expandEndpointShorthand(endpoint));
    }, []).map(originOf);
  ...
  var RiskIntelligenceHandle = /*#__PURE__*/function () {
    /**
     ...
     * @internal
     */
    function RiskIntelligenceHandle(opts) {
      _classCallCheck(this, RiskIntelligenceHandle);
      /**
    ...
    _createClass(RiskIntelligenceHandle, [{
      key: "handleStartMode",
      value: function handleStartMode() {
        var _this6 = this;
      ...
      value: function request() {
        ...
        this.requestRiskIntelligence().then(function (data) {
          ...
          if (_this7.hiddenFormEl) {
            _this7.hiddenFormEl.value = data.token;
          }
          fireFRCEvent(_this7.e, {
  ...
  function getRetrySrc(src, nextOrigin, retryCount) {
    var srcOrigin = originOf(src);
    var normalizedNextOrigin = originOf(nextOrigin);
    var pathAndQuery = stringHasPrefix(src, srcOrigin) ? src.slice(srcOrigin.length) : src;
    if (pathAndQuery.length === 0) {
      pathAndQuery = "/";
    } else if (!stringHasPrefix(pathAndQuery, "/")) {
      pathAndQuery = "/" + pathAndQuery;
    }
    var separator = pathAndQuery.indexOf("?") === -1 ? "?" : "&";
  ...
  var FriendlyCaptchaSDK = /*#__PURE__*/function () {
    function FriendlyCaptchaSDK() {
      ...
      if (opts.startAgent) {
        var origins = resolveAPIOrigins(this.apiEndpoint || getSDKAPIEndpoint());
        var retryOrigins = this.getRetryOrigins(origins);
    ...
    _createClass(FriendlyCaptchaSDK, [{
      key: "getRetryOrigins",
      value: function getRetryOrigins(origins) {
        return _getRetryOrigins(origins);
      }
    }, {
      ...
      key: "handleRiskIntelligenceMessage",
      value: function handleRiskIntelligenceMessage(msg) {
        if (msg.type === "root_risk_intelligence_generate_reply") {
          var promise = this.riskIntelligencePromises.get(msg.uid);
          if (promise) {
            if (msg.data) {
          ...
          var _promise = this.clearRiskIntelligencePromises.get(msg.uid);
          if (_promise) {
            if (msg.error) {
              _promise.reject(msg.error);
            } else {
              _promise.resolve();
            }
          } else {
      ...
      value: function ensureAgentIFrame(retryOrigins) {
        var _this9 = this;
        var attempt = 1;
        var originIndex = getRetryOriginIndex(attempt, retryOrigins);
        var origin = retryOrigins[originIndex];
        var src = origin + agentEndpoint;
        var maxAttempts = MAX_IFRAME_LOAD_ATTEMPTS;
        var existing = this.agents.get(origin);
        ...
        var registerWithRetry = function registerWithRetry() {
          _this9.bus.registerTargetIFrame("agent", agentId, el, _this9.getRetryTimeout(attempt)).then(function (status) {
            if (status === "timeout") {
              if (attempt >= maxAttempts) {
                console.error("[Friendly Captcha] Failed to load agent iframe after ".concat(attempt - 1, " retries."));
                el.remove();
                _this9.agents["delete"](origin);
                return;
              }
              var nextAttempt = attempt + 1;
              var nextIndex = getRetryOriginIndex(nextAttempt, retryOrigins);
      ...
      value: function setupPeriodicRefresh() {
        ...
        setInterval(function () {
          var e = "&expire=" + count++;
          _this10.agents.forEach(function (el, origin) {
            el.src += e;
          });
          _this10.widgets.forEach(function (w, id) {
      ...
      value: function attach(elements) {
        var _this11 = this;
        var _findFRCElements = findFRCElements(),
          _findFRCElements2 = _slicedToArray(_findFRCElements, 2),
          captchaElements = _findFRCElements2[0],
          riskIntelligenceElements = _findFRCElements2[1];
        var _loop2 = function _loop2() {
          var hElement = riskIntelligenceElements[index];
          if (hElement && !hElement.frcRiskIntelligence) {
            var _ds = hElement.dataset;
            var sitekey = _ds.sitekey;
            if (!sitekey) {
              console.warn("Risk Intelligence <div> found with no sitekey, skipping...", hElement);
              return 1; // continue
            }
            _this11.riskIntelligenceHandles.push(new RiskIntelligenceHandle({
              element: hElement,
              formFieldName: _ds.formFieldName,
              startMode: _ds.start,
              riskIntelligence: function riskIntelligence() {
                return _this11.riskIntelligence({
                  sitekey: sitekey,
                  apiEndpoint: _ds.apiEndpoint
                });
              }
            }));
          }
        };
        for (var index = 0; index < riskIntelligenceElements.length; index++) {
          if (_loop2()) continue;
        }
        if (elements === void 0) {
      ...
      value: function createWidget(opts) {
        var _this12 = this;
        var origins = resolveAPIOrigins(opts.apiEndpoint || this.apiEndpoint || getSDKAPIEndpoint());
        var retryOrigins = this.getRetryOrigins(origins);
        var attempt = 1;
        this.bus.addOrigins(origins);
        var origin = retryOrigins[getRetryOriginIndex(attempt, retryOrigins)] || origins[0];
        var agentId = this.ensureAgentIFrame(retryOrigins);
        ...
        function setUnreachableState(detail) {
          var debugString = encodeStringToBase64Url(JSON.stringify({
            sdk_v: "1.0.0",
            sitekey: opts.sitekey || "",
            retry: attempt + "",
        ...
        var registerWithRetry = function registerWithRetry() {
          _this12.bus.registerTargetIFrame("widget", widgetId, wel, _this12.getRetryTimeout(attempt)).then(function (status) {
            if (status === "timeout") {
              if (attempt >= maxAttempts) {
                console.error("[Friendly Captcha] Failed to load widget iframe after ".concat(attempt - 1, " retries."));
              ...
              widgetPlaceholder.textContent = getLocalizedPlaceholderText(language, "retrying") + " (".concat(attempt, ")");
      ...
      value: function riskIntelligence(opts) {
        var origins = resolveAPIOrigins(opts.apiEndpoint || this.apiEndpoint || getSDKAPIEndpoint());
        var retryOrigins = this.getRetryOrigins(origins);
        this.bus.addOrigins(origins);
        var agentId = this.ensureAgentIFrame(retryOrigins);
        ...
        var riskIntelligencePromise = flatPromise();
      ...
      key: "clearRiskIntelligence",
      value: function clearRiskIntelligence(opts) {
        var origins = resolveAPIOrigins((opts == null ? void 0 : opts.apiEndpoint) || this.apiEndpoint || getSDKAPIEndpoint());
        var retryOrigins = this.getRetryOrigins(origins);
        this.bus.addOrigins(origins);
        var agentId = this.ensureAgentIFrame(retryOrigins);
        var uid = randomId(8);
        ...
        var clearRiskIntelligencePromise = flatPromise();
      ...
      key: "getAllRiskIntelligenceHandles",
      value: function getAllRiskIntelligenceHandles() {
        var out = [];
        this.riskIntelligenceHandles.forEach(function (rih) {
          out.push(rih);
        });
        return out;
      }
